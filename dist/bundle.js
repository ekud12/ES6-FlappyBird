...
new Promise(function(resolve) { resolve(); }).then(function() { var __WEBPACK_AMD_REQUIRE_ARRAY__ = [!(function webpackMissingModule() { var e = new Error("Cannot find module \"canvasPainter\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()), !(function webpackMissingModule() { var e = new Error("Cannot find module \"playersManager\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()), !(function webpackMissingModule() { var e = new Error("Cannot find module \"sharedConstants\""); e.code = 'MODULE_NOT_FOUND'; throw e; }())]; ((function (canvasPainter, PlayersManager, Const) {
  var enumState = {
    Login: 0,
    WaitingRoom: 1,
    OnGame: 2,
  ...
  function loadGameRoom() {
    ...
    _socket.emit('say_hi', nick, function (serverState, uuid) {
      ...
      if (serverState == enumState.OnGame) {
        ...
        if (_isTouchDevice == false) infoPanel(true, 'Press <strong>space</strong> to fly !', 3000);else infoPanel(true, '<strong>Tap</strong> to fly !', 3000);
      }
    });
  ...
  function displayRanking(score) {
    ...
    // Set medal !
    if (score.rank == 1) nodeMedal.classList.add('winner');else if (score.rank == 2) nodeMedal.classList.add('second');else if (score.rank == 3) nodeMedal.classList.add('third');
  ...
  function infoPanel(isShow, htmlText, timeout) {
    ...
    if (isShow == false) {
      ...
      if (timeout) _infPanlTimer = setTimeout(function () {
        infoPanel(false);
      }, timeout);
  ...
  if (window.navigator.msPointerEnabled) _isTouchDevice = true;else if ('ontouchstart' in window) _isTouchDevice = true;else _isTouchDevice = false;
...
var _parallax = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"parallax\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));

var _parallax2 = _interopRequireDefault(_parallax);

var _backgroundRessources = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"backgroundRessources\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
...
function drawPipe(pipe) {
  ctx.drawImage(_picPipe, 0, 0, SPRITE_PIPE_WIDTH, SPRITE_PIPE_HEIGHT, pipe.posX, pipe.posY - SPRITE_PIPE_HEIGHT, _global2.default.PIPE_WIDTH, SPRITE_PIPE_HEIGHT);
  ctx.drawImage(_picPipe, 0, 0, SPRITE_PIPE_WIDTH, SPRITE_PIPE_HEIGHT, pipe.posX, pipe.posY + _global2.default.HEIGHT_BETWEEN_PIPES, _global2.default.PIPE_WIDTH, SPRITE_PIPE_HEIGHT);
}
...
that.draw = function (currentTime, ellapsedTime, playerManager, pipes, gameState, isNight) {
  ...
  if (pipes) _parallaxGround.draw(ctx, currentTime);else _parallaxGround.draw(ctx, 0);
};
...
that.loadRessources = function (onReadyCallback) {
  ...
  _parallaxGround = new _parallax2.default(_picGround, null, 900, 96, _global2.default.LEVEL_SPEED, 672, _global2.default.SCREEN_WIDTH);
  _picPipe = new Image();
  ...
  for (i = 0; i < _backgroundRessources2.default.length; i++) {
    ...
    _picBG.push(new _parallax2.default(dBg, nBg, _backgroundRessources2.default[i].width, _backgroundRessources2.default[i].height, _backgroundRessources2.default[i].speed, _backgroundRessources2.default[i].posY, _global2.default.SCREEN_WIDTH));
  }
...
var _playerEntity = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"playerEntity\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
