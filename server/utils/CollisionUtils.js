...
const checkSingleCollision = (vine, toucanLatestProps) => {
  ...
  if (toucan.posX + Config.TOUCAN_RENDER_WIDTH > vine.posX && toucan.posX < vine.posX + Config.VINE_WIDTH) {
    toucanLatestProps.updateScore(vine.id);
    if (toucan.posY < vine.posY) return true;
    if (toucan.posY + Config.TOUCAN_RENDER_HEIGHT > vine.posY + Config.HEIGHT_BETWEEN_VINES) {
      return true;
    }
  }
