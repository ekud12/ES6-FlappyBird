...
class VineManager {
  ...
  newVine() {
    ...
    if (_vineList.length > 0) lastPos = _vineList[_vineList.length - 1].getVineObject().posX;
  ...
  updateVines(time) {
    ...
    if (_vineList[nbVines - 1].getVineObject().posX < SPAWN_VINE_ALERT) this.emit('need_new_vine');
  }
